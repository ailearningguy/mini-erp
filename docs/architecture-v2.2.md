# ERP Architecture v2.2 (Canonical)

**Version:** 2.2  
**Date:** 2026-04-01  
**Status:** Canonical Source of Truth  
**Supersedes:** architecture-v2.1.md (deprecated), architecture-v2.md (deprecated), architecture-v1.md (deprecated), architecture-design.md (deprecated), architecture-design-short.md (draft)  
...
## ADR-008: Event Schema Registry → Producer + Consumer Validation (NEW in v2.2)

### Decision

All events MUST be validated against a registered schema before publishing and after receiving. The Event Schema Registry maintains versioned schemas using JSON Schema or Zod.
...
type ProductCreatedEvent = z.infer<typeof ProductCreatedEventSchema>;
...
class EventSchemaRegistry {
  ...
  validate(eventType: string, data: unknown): z.infer<z.ZodSchema> {
...
- Prevents schema drift between producer and consumer
- Enables fail-fast on malformed events
- Provides clear contract documentation

### Consequences

- ✅ Producer MUST validate event against registered schema before emitting
- ✅ Consumer MUST validate received event against registered schema before processing
- ✅ New event versions MUST register new schema
- ❌ No event publishing without registered schema

---

## ADR-009: Migration Rule Enforcement → Linter + CI Check (NEW in v2.2)

### Decision

Migration rules are enforced via a migration linter that runs as part of CI pipeline.

```typescript
// migration-linter.ts
interface MigrationRule {
  name: string;
  check: (migration: Migration) => ValidationResult;
}
...
const migrationRules: MigrationRule[] = [
  {
    ...
    check: (migration) => {
      const violations = migration.statements
        .filter(stmt => stmt.type === 'ALTER_TABLE_ADD_COLUMN')
        .filter(col => col.constraints.includes('NOT NULL') && !col.default)
        .map(col => ({
          column: col.name,
  ...
  },
  {
    ...
    check: (migration) => {
      const hasTimeout = migration.statements.some(
        stmt => stmt.sql.includes('SET statement_timeout')
      );
      return {
        valid: hasTimeout,
        violations: hasTimeout ? [] : [{ error: 'Migration MUST set statement_timeout' }],
      };
    },
  },
  {
    ...
    check: (migration) => {
      const destructiveStatements = ['DROP COLUMN', 'DROP TABLE', 'DROP INDEX'];
      const violations = migration.statements
        .filter(stmt => destructiveStatements.some(ds => stmt.sql.includes(ds)))
        .map(stmt => ({ sql: stmt.sql, error: 'Destructive statements require manual review' }));
...
class MigrationLinter {
  async lint(migrationFile: string): Promise<LintResult> {
    const migration = this.parseMigration(migrationFile);
    const results = migrationRules.map(rule => rule.check(migration));

    const allValid = results.every(r => r.valid);
    return {
      valid: allValid,
      errors: results.flatMap(r => r.violations),
    };
  }
}
```
...
- Rule "NEVER add NOT NULL column without default" was only documentation
- Enforcement prevents accidental deployment-blocking migrations
- Automated in CI before migration can run

### Consequences

- ✅ CI pipeline fails if any migration lint rule fails
- ✅ Migration files are validated before `drizzle-kit migrate` runs
- ✅ All rules from Section 5 are enforced programmatically
...
### Phase 1 Security Clarification (ENHANCED in v2.2)

> **CRITICAL:** Phase 1 (in-process) plugins are **TRUSTED ONLY**. The system does NOT provide security isolation in Phase 1.

```typescript
interface PluginMetadata {
  // ...
  /**
   * PHASE 1 PLUGINS ARE TRUSTED ONLY
   * - Plugin CAN access Node.js globals (Buffer, fs, process.env)
   * - Plugin crash CAN crash main process
   * - No memory isolation or resource enforcement
   *
   * Only install plugins from trusted sources.
   * For untrusted plugins, wait for Phase 2+ isolation.
   */
  trusted?: boolean;  // Default: true for Phase 1, must be explicitly set to false
}
```
...
- **ENFORCED via migration linter** (ADR-009) — CI fails if rules violated
...
- [ ] Migration linter passes (ADR-009)
...
| Rate Limiting | Per-event-type rate limit (NEW in v2.2) |

### Exact Processing Flow (Consumer Side)

```
1. Consumer receives message from RabbitMQ
2. Deserialize and validate event schema (ADR-008)
3. Check processed-event store for event.id
   → EXISTS: ACK message, skip (dedup)
   → NOT EXISTS: continue
4. Check rate limit for event type → reject if exceeded
5. Enqueue onto per-aggregate_id PQueue (sequential ordering)
6. Execute handler:
...
### Rate Limiting (NEW in v2.2)

```typescript
interface EventRateLimitConfig {
  eventType: string;
  maxEventsPerSecond: number;
  burstAllowance?: number;  // Allow short bursts above limit
}

const defaultEventRateLimits: EventRateLimitConfig[] = [
...
class EventRateLimiter {
  ...
  constructor(private config: EventRateLimitConfig[]) {
    for (const cfg of config) {
      this.limiters.set(cfg.eventType, new TokenBucket(cfg.maxEventsPerSecond, cfg.burstAllowance));
    }
  }

  async checkLimit(eventType: string): Promise<boolean> {
    const limiter = this.limiters.get(eventType);
    if (!limiter) return true; // No limit configured for this event type
...
**Why rate limiting:**
- Prevents event storm from overloading downstream services
- Protects against misbehaving producers
- Provides back-pressure mechanism
...
| `event_rate_limited_total` | Counter | `type` | High rate (producer misbehaving) |
...
## Saga Retention Strategy (NEW in v2.2)

Saga state table requires retention policy to prevent unbounded growth.

```typescript
// Schema extended with retention fields
export const sagaState = pgTable('saga_state', {
  ...
  // NEW in v2.2
  ttlAt: timestamp('ttl_at'),  // Time-to-live for automatic cleanup
});

// Retention policy
const sagaRetentionPolicy = {
  COMPLETED: { retainDays: 30, archiveBeforeDelete: true },
  FAILED: { retainDays: 90, archiveBeforeDelete: true },
  COMPENSATING: { retainDays: 90, archiveBeforeDelete: true },
};

// Cleanup job runs daily
class SagaCleanupJob {
  @Cron('0 2 * * *') // 2 AM daily
  async cleanup(): Promise<CleanupReport> {
    const cutoff = new Date();
    const sagasToArchive = await this.sagaRepo.findByStatusAndTtl([
      'COMPLETED', 'FAILED', 'COMPENSATING'
    ], cutoff);

    for (const saga of sagasToArchive) {
      await this.archiveSaga(saga);
      await this.sagaRepo.delete(saga.id);
    }

    return { archived: sagasToArchive.length };
  }
}
```

| Saga Status | Retention | Action |
|-------------|-----------|--------|
| `RUNNING` | N/A | Never delete while in progress |
| `COMPENSATING` | 90 days | Archive to cold storage, then delete |
| `COMPLETED` | 30 days | Archive to cold storage, then delete |
| `FAILED` | 90 days | Archive, requires manual review before delete |
...
```typescript
/**
 * Phase 1 plugins run in the same process as the main application.
 * This means:
 *
 * - Plugin CAN access Node.js globals (Buffer, fs, process.env, etc.)
 * - Plugin CAN read other plugin's memory (if reference leaked)
 * - Plugin crash (uncaught exception) CAN crash the main process
 * - No memory isolation or CPU enforcement (limits are soft/monitored)
 *
...
interface PluginMetadata {
  // ...
  /**
   * For Phase 1: default is TRUE
   * - trusted: true  → Allow installation, assume safe
   * - trusted: false → Block installation until Phase 2 isolation available
   */
  trusted?: boolean;
}
```
...
⚠️ TRUSTED ONLY            Hard limits enforced      Full sandbox
```
...
│  ⚠️ SECURITY MODEL: TRUSTED PLUGINS ONLY         │
└────────────────────────────────────────────────┘
```
...
interface PluginMetadata {
  ...
  trusted?: boolean;                     // NEW in v2.2 - Phase 1 must be trusted
  // ... existing fields
}
```
...
## Cache Stampede Protection (NEW in v2.2)

When cache misses occur and multiple concurrent requests all hit the database simultaneously, use mutex or request coalescing to prevent thundering herd.

```typescript
class CacheService {
  private mutexes = new Map<string, Promise<any>>();
  ...
  async getOrSet<T>(key: string, factory: () => Promise<T>, ttl: number): Promise<T> {
    ...
    let pending = this.mutexes.get(key);
    if (pending) {
...
> **Note:** Cache race condition (invalidate while read in progress) acceptable within TTL window, but stampede protection is RECOMMENDED for high-traffic keys.

### Alternative: Redis SETNX Lock

For distributed environment where in-process mutex doesn't work:

```typescript
class DistributedCacheLock {
  async acquireLock(key: string, ttlMs: number = 5000): Promise<boolean> {
    const result = await this.redis.set(`lock:${key}`, '1', 'NX', 'PX', ttlMs);
    return result === 'OK';
  }
  ...
  async withLock<T>(key: string, fn: () => Promise<T>): Promise<T> {
    ...
    throw new LockFailedError(); // Let caller retry via cache
  }
}
```
...
- Cache race condition: acceptable within TTL, but stampede protection RECOMMENDED (v2.2)
...
- **Phase 1: TRUSTED ONLY** (v2.2 clarification)
- Enforcement points:
...
### Push-Based Option (NEW in v2.2)

PostgreSQL LISTEN/NOTIFY can be used for near-real-time event delivery instead of polling.

```typescript
// Enable notifications for low-latency delivery
const outboxNotify = pgTable('outbox_notify', {
  id: uuid('id').defaultRandom().primaryKey(),
...
> **Note:** This is an **OPTIMIZATION** — the standard polling approach (Section 6) remains the default. Push-based LISTEN/NOTIFY can be enabled for lower latency.
...
6. **Push notification ready**: Optional PostgreSQL LISTEN/NOTIFY for low-latency delivery (v2.2)
...
- Install untrusted plugin in Phase 1 (v2.2)
- Emit event without schema validation (v2.2)
- Run migration without linter check (v2.2)
...
- [ ] Event schema registered and validated (ADR-008)
...
- [ ] Plugin `trusted` flag set correctly for Phase 1 (v2.2)
...
11. **Fail-safe cache** - Stampede protection optional but recommended (v2.2)
...
|----------|--------|---------|
| architecture-v2.2.md | **Canonical** | Source of Truth |
| architecture-v2.1.md | **Deprecated** | Superseded by v2.2 |
...
# 38. Production Hardening Checklist (v2 + v2.1 + v2.2)
...
- [ ] **Outbox push notification option** (PostgreSQL LISTEN/NOTIFY, v2.2)
...
### Event Processing (NEW in v2.1 + v2.2)
...
- [ ] **Event schema registry and validation (ADR-008, v2.2)**
- [ ] **Rate limiting per event type (v2.2)**

### Saga (NEW in v2.1 + v2.2)
...
- [ ] **Saga retention policy (30 days COMPLETED, 90 days FAILED, v2.2)**
...
- [ ] **Phase 1 `trusted` flag enforcement (v2.2)**
...
- [ ] Health checks (liveness + readiness)
- [ ] **Migration linter in CI (ADR-009, v2.2)**

### Cache (v2.2)

- [ ] Cache stampede protection (optional but recommended)
- [ ] Cache race condition acceptable within TTL
...
# 41. v2.1 → v2.2 Changelog (NEW)

## Added (NEW in v2.2)

| Section | What | Why |
|---------|------|-----|
| ADR-008 | **Event Schema Registry** — Producer + Consumer validation with Zod | Prevents schema drift between producer/consumer, enables fail-fast on malformed events |
| ADR-009 | **Migration Rule Enforcement** — Linter + CI check (ADR-009) | Migration rule "NEVER add NOT NULL without default" was only documentation — now enforced in CI |
| Section 1 | **Core Principle 9** — "Fail-safe cache" | Cache race conditions known but acceptable, stampede protection recommended |
| Section 6 | **Event Rate Limiting** — Per-event-type rate limit | Prevents event storm from overloading downstream services |
| Section 6 | **Event Processing Flow** — Added schema validation step (ADR-008) + rate limit check | Ensures every event is validated before processing |
| Section 10 | **Saga Retention Policy** — 30 days COMPLETED, 90 days FAILED | Saga state table grows unbounded — requires retention policy |
| Section 12 | **Phase 1 Security Clarification** — "TRUSTED ONLY" | Phase 1 plugins run in same process — must be trusted. Document explicitly warns |
| Section 13 | **Phase 1 Hardening** — `trusted` flag in metadata | Allows system to block untrusted plugins in Phase 1 |
| Section 19 | **Cache Stampede Protection** — Request coalescing + Redis SETNX lock | Thundering herd problem when cache misses — optional but recommended |
| Section 32 | **Push-Based Outbox Option** — PostgreSQL LISTEN/NOTIFY | Polling latency can be high — optional push notification for low-latency |
| Section 32 Anti-Patterns | **3 new anti-patterns** | Install untrusted plugin, emit without schema validation, run migration without linter |
| Section 33 Checklist | **4 new checklist items** | Event schema validation, plugin trusted flag, migration linter, stampede protection |
| Section 38 | **4 new subsections** | Outbox push notification, Event schema registry, Rate limiting, Saga retention policies |
| Section 36 | v2.2 is now canonical, v2.1 deprecated |

## Changed (v2.2)

| Section | Change |
|---------|--------|
| Header | Version 2.1 → 2.2 |
| Section 1 | Added principle 9: "Fail-safe cache" |
| Section 6 | Added event schema validation + rate limiting subsections |
| Section 10 | Added saga retention strategy with TTL |
| Section 12 | Added Phase 1 security clarification |
| Section 19 | Added cache stampede protection + alternative Redis lock |
| Section 32 | Added push-based notification option |
| Section 37 CI | Added migration linter to CI pipeline |

## Addressed Review Comments (v2.2)

| Review Comment | ADR/Section | Resolution |
|----------------|-------------|------------|
| Outbox polling chưa tối ưu | Section 32 | Added PostgreSQL LISTEN/NOTIFY push option |
| Event schema thiếu schema registry | ADR-008 | Added event schema registry with Zod validation |
| Plugin security (Phase 1) còn yếu | Section 12 | Added "TRUSTED ONLY" clarification + trusted flag |
| Missing: Rate limiting cho Event Consumer | Section 6 | Added per-event-type rate limiting |
| Saga persistence: chưa đề cập cleanup / TTL | Section 10 | Added 30/90 day retention policy |
| Redis cache consistency: có risk race condition | Section 19 | Added stampede protection (optional but recommended) |
| Migration rule chưa có enforcement tool | ADR-009 | Added migration linter + CI check |

## Preserved from v2.1

All v2.1 content preserved as-is. Only additions and targeted enhancements above.

## Design Principles for v2.2

1. **Review-driven improvements** — each addition addresses specific review comments
2. **Optional optimizations marked** — push notifications, stampede protection are RECOMMENDED not required
3. **Security first** — Phase 1 now explicitly documented as "trusted only"
4. **Operational awareness** — saga retention, migration enforcement prevent operational debt
5. **Backward compatible** — all v2.1 patterns work unchanged

---

# 42. v2.2 Scoring Summary

| Review Dimension | v2.1 Score | v2.2 Improvement |
|------------------|------------|------------------|
| Architecture correctness | ⭐⭐⭐⭐⭐ | — |
| Production readiness | ⭐⭐⭐⭐⭐ | — |
| Scalability | ⭐⭐⭐⭐☆ | **+0.5** (rate limiting, push notification) |
| Security | ⭐⭐⭐⭐☆ | **+0.5** (Phase 1 trusted clarification) |
| Maintainability | ⭐⭐⭐⭐⭐ | — |
